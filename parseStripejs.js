...
const PATHS = fs.readdirSync('DefinitelyTyped/types/stripejs')
  .map(x => `DefinitelyTyped/types/stripejs/${x}`)
  .filter(x => fs.statSync(x).isFile())
  .filter(x => !x.endsWith('-tests.ts'))
  .filter(x => x.match(/\.ts$/))
...
PATHS.forEach(path => {
  ...
  OUTPUT_PATH.pop()
  OUTPUT_PATH.pop()
  OUTPUT_PATH.forEach((x, i) => {
    if (x === 'stripejs/index') {
      OUTPUT_PATH[i] = 'stripejs'
    }
  })
  ...
  const write = writes[path2] = writes[path2] ?? { i: [], t: [], b: [] }
  write.i.push('\n', ...finalImportText)
  write.t.push('\n', ...t)

  code.replace(/\/\/\/\s+<reference\s+lib="([^"]+)"/g, (_, $1) => {
    write.b.push(`bear @drumwork/moon/code/javascript/${$1.replace(/\./g, '/')}`)
  })
})
...
function makeExportNamedImports(node, mod, declared) {
...
function handleBody(node, mod, declared, isTop = false, isGlobal, hostsAndTasks) {
  switch (node.type) {
    case 'TSInterfaceDeclaration':
...
function makeClass(node, mod, declared, isGlobal) {
  const imports = {}
  const heads = {}
    ...
    switch (node.type) {
      case 'TSDeclareMethod': {
        ...
        if (node.kind === 'constructor') {
          ...
          if (node.computed) {
            ...
            if (`  task ${methodName}, name <${node.key.name}>`.length <= 96) {
              t.push(`  task ${methodName}, name <${node.key.name}>`)
            } else {
              t.push(`  task ${methodName}`)
              t.push(`    name <${node.key.name}>`)
            }
          }
          typeParams.forEach(line => {
            t.push(`    ${line}`)
      ...
      case 'ClassProperty': {
        ...
        if (node.computed) {
          ...
          t.push(`  take ${name}, name <${node.key.name}>`)
          if (type.length === 1) {
            t[t.length - 1] += `, ${type[0]}`
          } else {
            type.forEach(line => {
...
function makeExportNamed(node, mod, declared, isGlobal, hostsAndTasks) {
  if (node.specifiers?.length) {
    // console.log(node)
...
function makeModule(node, mod, declared, isTop, isGlobal, hostsAndTasks) {
  const name = makeName(node.id.name ?? node.id.value)
  if (name.match(/^node\-/)) return
  isGlobal = isGlobal || (name === 'global')
  let n = isGlobal ? null : (name === OUTPUT_PATH.split('/').pop()) ? null : `${name}`
  if (node.body.type === 'TSModuleDeclaration') {
    makeModule(node.body, isTop ? mod : [mod, n].filter(x => x).join('/').replace(/google\/maps\/?/, ''), declared, false, isGlobal, hostsAndTasks)
  } else {
...
function makeModuleImports(node, mod, declared, isTop, isGlobal) {
  const name = makeName(node.id.name ?? node.id.value)
  isGlobal = isGlobal || name === 'global'
  let n = isGlobal ? null : (OUTPUT_PATH.split('/').pop() === name) ? null : name
...
function makeAlias(node, mod, declared, isGlobal) {
  const typeName = node.id.name
  const imports = {}
...
function makeInterface(node, mod = '', declared, isGlobal) {
  const typeName = node.id.name
  const _typeName = makeName(typeName)
  const imports = {}
...
function makeIndex(typeName, node, imports, heads) {
  ...
  makeIdentifier(typeName, node.parameters[0], 'mesh', imports, heads).forEach(line => {
    t.push(`  ${line}`)
  })
...
function makeProperty(node, imports, heads) {
  ...
  switch (node.key.type) {
    ...
    default: {
      ...
      if (`  take ${makeName(name)}, name <${name}>`.length <= 96) {
        t.push(`  take ${makeName(name)}, name <${name}>`)
      } else {
        t.push(`  take ${makeName(name)}`)
        t.push(`    name <${name}>`)
      }
      const like = makeTypeAnnotation(name, node.typeAnnotation.typeAnnotation, 'like', imports, propHeads)
      like.forEach(line => {
...
function makeTypeAnnotation(name, node, type = 'like', imports, heads) {
  ...
  switch (node.type) {
    case 'TSStringKeyword':
      ...
      break
    case 'TSArrayType':
      imports['native-array'] = true
      text.push(`${type} native-array`)
...
function makeFunctionParams(name, node, imports, heads) {
  ...
  ;(node.parameters ?? node.params).forEach(node => {
    switch (node.type) {
      ...
      case 'ObjectPattern':
        let xName = `x${x++}`
        text.push(`take ${xName}, like form-link-text`)
        imports['form-link-text'] = `load @drumwork/seed/code/form/form\n  take form form\n    save form form-link-text`
        makeTypeAnnotation(name, node.typeAnnotation.typeAnnotation, 'like', imports, heads).forEach(line => {
          text.push(`  ${line}`)
        })
        imports['form-link-text'] = `load @drumwork/seed/code/form/form\n  take form form\n    save form form-link-text`
