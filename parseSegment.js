...
const PATHS = ['DefinitelyTyped/types/segment-analytics/index.d.ts']
...
PATHS.forEach(path => {
  ...
  OUTPUT_PATH.forEach((x, i) => {
    if (x === 'segment-analytics/index') {
      OUTPUT_PATH[i] = 'segment-analytics'
    }
  })
...
Object.keys(writes).forEach(path => {
  mkdir(path)
  const { i, t } = writes[path]
  let imports = Object.keys(i.reduce((m, x) => m[x] = true && m, {})).sort()
  fs.writeFileSync(`${path}/base.link`, cleanText(imports.join('\n\n') + '\n' + t.join('\n')))
})
...
function makeClass(node, mod, declared, isGlobal, ns) {
  ...
  if (isGlobal) text.push(`  home take`)
...
function makeModule(node, mod, declared, isTop, isGlobal, hostsAndTasks, ns) {
  ...
  let n = isGlobal ? null : name
  if (node.body.type === 'TSModuleDeclaration') {
    makeModule(node.body, isTop ? mod : [mod, n].filter(x => x).join('/').replace(/segment-analytics\/?/, ''), declared, false, isGlobal, hostsAndTasks, [ns, NAME].filter(x => x).join('.'))
  } else {
    const mods = isTop ? n : [mod, n].filter(x => x).join('/').replace(/segment-analytics\/?/, '')
    const ht = {
      imports: {},
      hosts: [],
      tasks: [],
...
function getImportText(imports, declared) {
  ...
  Object.keys(imports).forEach(key => {
    ...
    if (x && x.fromFile) {
      ...
      val = `load @drumwork/${parts.match('google') ? 'snow' : 'land'}/code/javascript/${parts}/${key}\n  take form ${key}`
    } else if (x === true) {
      let v
      let parts
...
function makeUnionType(name, node, type = 'like', imports, heads) {
  const text = [`${type} sink-link-text`]
  imports['sink-link-text'] = `load @drumwork/seed/code/form/sink\n  take form sink\n    save form sink-link-text`
    ...
    case 'TSTypeOperator':
      switch (node.operator) {
        case 'readonly':
          ...
          // do nothing, since it is lock free in our situation already.
          break
        case 'unique':
          text.push(...makeTypeAnnotation(name, node.typeAnnotation, type, imports, heads))
          text.push(`  cool take`)
